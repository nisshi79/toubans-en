        ...
        fetch('getToubanList.php?groupId=test')
            ...
            .then(json => {
                json.forEach(function (touban) {
                    $('.toubanList')
                        .append(`<li><a class="black_font large_font">${touban.role[0].role}: </a><a class="btn" href="mainForm.php?groupId=test&tableId=${touban.table.id}">Edit</a> <a id="${touban.table.id}" class="btn touban_del" href="javascript:void(0)">削除</a></li>`);
                    $('.toubanList').off('click');
                    $('.toubanList').on('click','li .touban_del', function () {
                        if(window.confirm('Are you sure to delete this duty?')){
...
function initializeApp(data) {
    ...
    fetch('getToubanList.php?groupId='+lineId)
        ...
        .then(json => {
            json.forEach(function (touban) {
                $('.toubanList')
                    .append(`<li><a class="black_font large_font">${touban.role[0].role}: </a><a class="btn" href="mainForm.php?groupId=${lineId}&tableId=${touban.table.id}">Edit</a> <a id="${touban.table.id}" class="btn touban_del" href="javascript:void(0)">削除</a></li>`);
                $('.toubanList').off('click');
                $('.toubanList').on('click','li .touban_del', function () {
                    if(window.confirm('Are you sure to delete the duty?')){
